...
import BlobImagePreview from "../../../components/apps/walrus_blob_storage/BlobImagePreview";
import { fetchBlobFromWalrus } from "../../../utils/helper";
...
export const WalrusBlobStorage = () => {
  ...
  React.useEffect(() => {
    // giả lập thời gian fetch data
    setTimeout(() => {
      // Lấy các data SUI ObjectId
      setFilteredData(dataWalnet);
    }, 2000)
  }, [])
  // Hàm để lọc dữ liệu dựa trên searchTerm
  const handleFilter = (value) => {
    const result = dataWalnet.filter(item => item.object_id.includes(value));
  ...
  const aggregateInfoTransaction = async (objectId) => {
    ...
    const blobData = await fetchBlobFromWalrus("gOsI3bwzcxh5ciJQJ1WCRCgxYupiXthnRJ9Eydo5734");
    const suiObject = {
      sui_object_id: "0x17bf0d3e636e95e78f9d1...1d40a1225bcc15a00b6315fe",
      blob_id: "ZU-ZSU7bXnuiPHcE2W08supJVO1b_pGgPNcQfDNHI48",
      file_type: "PNG",
      size: 9300, // kích thước tính bằng byte
      certified_epoch: 0,
      stored_epoch: 0,
      blob_data: blobData
    };
                ...
                filteredData ?
                  <>
                    {
                      filteredData.map((i) => {
                        return (
                          <div
                            key={i.object_id}
                            className={`navLink ${i.object_id === objectIdSelected ? "selected" : ""}`}
                            onClick={() => aggregateInfoTransaction(i.object_id)}
                          >
                            <p className="content-item-traction truncate w-full">{i.object_id}</p>
                          </div>
                        );
                      })
                    }
                  ...
                  (
                    Array.from({ length: 10 }).map((_, index) => (
                      <div key={index} className={"navLink animate-pulse unselected"}>
                        <div
            ...
            </div>
            {
              ...
              >
                <div className="flex flex-row items-center my">
                  <p className="me-2 my-2">Upload file</p>
                  <svg xmlns="http://www.w3.org/2000/svg" class="w-7 mb-1 fill-neutral-700 dark:fill-neutral-200" viewBox="0 0 32 32">
                    <path
...

          {
            !navToUploadFile ?
              (objectIdSelected &&
                filteredData.map((i) => {
                  return (
                    objectIdSelected === i.object_id && (
                      <main key={i.object_id}>
                        <div className="flex flex-col w-full h-full tilesCont win11Scroll">

                          <p className="title-preview text-2xl mt-0">Preview Image</p>
                          {
                            !dataPreviewImage ?
                              <div>
                                <div className="flex items-center justify-center">
                                  <div role="status" class="flex items-center justify-center h-[250px] w-[800px] max-w-sm bg-gray-300 rounded-lg animate-pulse dark:bg-white/20">
                                    <svg class="w-10 h-10 text-white/50" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 18">
                              ...
                              <div>
                                <div className="flex items-center justify-center">
                                  ...
                                  <BlobImagePreview
                                    ...
                                    blobData={dataPreviewImage?.blob_data}
                                    alt="preview-image"
                                ...
                                <div class="mt-10">
                                  <div class="space-y-4">
                                    <div class="flex justify-between">
                                      ...
                                      <span>{dataPreviewImage?.sui_object_id}</span>
                                    </div>
                                    <div class="flex justify-between">
                                      <span class="font-semibold">Blob ID</span>
                                      <span>{dataPreviewImage?.blob_id}</span>
                                    </div>
                                    <div class="flex justify-between">
                                      <span class="font-semibold">File type</span>
                                      <span>{dataPreviewImage?.file_type}</span>
                                    </div>
                                    <div class="flex justify-between">
                                    ...
                                    <div class="flex justify-between">
                                      ...
                                      <span>{dataPreviewImage?.certified_epoch}</span>
                                    </div>
                                    <div class="flex justify-between">
                                      <span class="font-semibold">Stored Epoch</span>
                                      <span>{dataPreviewImage?.stored_epoch}</span>
                                    </div>
                                  </div>
                                </div>
                              </div>
              ...
              (<main>
                <div className="flex flex-col w-full h-full tilesCont win11Scroll">
                  <p className="title-preview text-2xl mt-0">Upload file</p>
                  ...
                  <div className="w-full flex flex-col items-center mb-4" >
                    <label for="uploadFile1"
                      ...
                      <svg xmlns="http://www.w3.org/2000/svg" class="w-11 mb-2 fill-neutral-700 dark:fill-neutral-200" viewBox="0 0 32 32">
                        <path
                          d="M23.75 11.044a7.99 7.99 0 0 0-15.5-.009A8 8 0 0 0 9 27h3a1 1 0 0 0 0-2H9a6 6 0 0 1-.035-12 1.038 1.038 0 0 0 1.1-.854 5.991 5.991 0 0 1 11.862 0A1.08 1.08 0 0 0 23 13a6 6 0 0 1 0 12h-3a1 1 0 0 0 0 2h3a8 8 0 0 0 .75-15.956z"
                          data-original="#000000" />
                        <path
                      ...
                      <p class="text-xs font-medium">PNG, JPG SVG, WEBP, and GIF are Allowed. (Max 10MiB Size)</p>
                  ...
                  <label htmlFor="epochs" className="mb-2">Choose epochs :</label>
                  <input
                    id="epochs"
                    className="input-epochs placeholder-white/50 w-[150px]"
